      ...
      return new Vue({
        ...
        data: {
          ...
          property: {
            ...
            quantity: 6,
            is_gril: 0,
            is_compound_surname: 0,
          },

        },
        mounted() {
          this.random_name(this.property)
        },
        methods: {
          /**
           * 随即从列表中选出指定数量的元素
           ...
           * @returns 元素列表
           */
          random_choice: function (list, quantity) {
            let result_list = []
            while (quantity > 0) {
           ...
           * @returns 姓名列表
           */
          // random_name: function (last_name, quantity, is_compound_surname, is_gril) {
          random_name: function (property) {
            let {
              quantity,
              is_compound_surname,
              is_gril
            ...
            while (quantity > 0) {
              // 姓
              let last_name = property.last_name
              if (last_name == "") {
                if (is_compound_surname) {
                  last_name = this.random_choice(this.last_name.compound_list, 1)
                } else {
                  last_name = this.random_choice(this.last_name.single_list, 1)
                }
              }
              ...
              let first_name = ''
              if (is_gril) {
                first_name = this.random_choice(this.first_name.girl_list, 2).join('')
              } else {
                first_name = this.random_choice(this.first_name.boy_list, 2).join('')
              }
              result_list.push(last_name + first_name)
              quantity--
            }
            this.name_list = result_list
            return result_list
          },
          insert_to_editor: function (str) {
            ct.editor.insert_string(str)
          },
          random_and_insert: function (property) {
            property = JSON.parse(JSON.stringify(property))
            property.quantity = 1
            let name = this.random_name(property)[0]
            this.insert_to_editor(name)
          }
        }
      })
    },
